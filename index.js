...
function addArtist(array, name, years, genre, nationality, bio) {
  ...
  copyArtists.push(updatedArtists);
  return copyArtists;
}

console.log(addArtist(artists, 'Dakota', '1999 - 2023', 'full stack web dev', 'american', 'this is a bio!'));
...
function lotsOfArt(array) {
  const prolificArtists = array.filter(array => array.paintings > 100); 
  const artistNames = prolificArtists.map(array => array.name); 
  return artistNames
}
...
function artistByCountry(array, nationality) {
  const filteredArtists = array.filter((artist) => {
    return (
      ...
      artist.nationality.split(",").length === 1
    );
  });
